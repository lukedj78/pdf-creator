...
import { useEffect, useCallback, useState, useId, useRef, useTransition } from "react"
import {
  DndContext,
  DragOverlay,
  useSensor,
...
import { generatePdfPreview } from "@/lib/actions/pdf"
...
export function EditorPageContent({ templateId }: { templateId: string | null }) {
  if (templateId) {
...
function EditorShell({
  ...
  const [previewMode, setPreviewMode] = useState<"spec" | "pdf">("spec")
  ...
  const handlePdfPreview = useCallback(() => {
    ...
    startPdfTransition(async () => {
      try {
        const { base64 } = await generatePdfPreview(store.state.template)
  ...
  }, [store.state.template, pdfBlobUrl])
    ...
    <EditorProvider store={store}>
      ...
      >
        ...
        <Dialog open={showPreview} onOpenChange={(open) => {
          ...
          <DialogContent className="!max-w-[100vw] !w-screen !h-screen !max-h-screen !rounded-none flex flex-col p-0 overflow-hidden">
            <DialogHeader className="px-6 pt-6 pb-2 flex flex-row items-center justify-between gap-4">
              <DialogTitle>Preview</DialogTitle>
              <div className="flex gap-1 rounded-lg border p-0.5">
                <Button
                  variant={previewMode === "spec" ? "secondary" : "ghost"}
                  size="sm"
                  onClick={() => setPreviewMode("spec")}
                >
                  Spec
                </Button>
                <Button
            ...
            <div className="flex-1 overflow-auto bg-muted/30 flex justify-center p-0">
              {previewMode === "spec" ? (
                ...
                <div className="flex items-center justify-center flex-1">
                  <div className="text-muted-foreground text-sm">Generating PDF...</div>
                </div>
              ) : pdfBlobUrl ? (
                <iframe
                  src={pdfBlobUrl}
                ...
                <div className="flex items-center justify-center flex-1">
                  <div className="text-muted-foreground text-sm">PDF generation failed</div>
                </div>
              )}
            </div>
          </DialogContent>
