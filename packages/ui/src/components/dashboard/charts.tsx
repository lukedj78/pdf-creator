...
const chartConfig = {
  expense: { label: "Expense", color: "var(--color-blue-500)" },
  profit: { label: "Profit", color: "var(--color-sky-400)" },
  earning: { label: "Earning", color: "var(--color-sky-400)" },
} satisfies ChartConfig;

export function SalesChart() {
  const legends = [
    { id: 1, title: "Earning", color: "bg-sky-400/50" },
    { id: 2, title: "Profit", color: "bg-sky-400" },
    { id: 3, title: "Expense", color: "bg-blue-500" },
  ];

  return (
    <Card className="w-full max-w-3xl mx-auto py-6 gap-8 shadow-xs">
      <CardHeader className="flex sm:flex-row flex-col justify-between sm:items-center items-start gap-3 px-6">
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <Badge className={cn("bg-teal-400/10 text-muted-foreground shadow-none")}>+18%</Badge>
      ...
      <CardContent className="px-6">
        <ChartContainer config={chartConfig} className="h-[300px] w-full">
          <BarChart accessibilityLayer data={chartData} margin={{ left: -20 }}>
            ...
            <YAxis tickLine={false} axisLine={false} tickMargin={10} fontSize={12} tickFormatter={(value) => `${value / 10}k`} domain={[0, 100]} ticks={[0, 50, 100, 150, 200, 250, 300]} />
            <ChartTooltip cursor={false} content={<ChartTooltipContent hideLabel indicator="line" />} />
            <Bar dataKey="expense" stackId="a" fill="var(--color-expense)" radius={[0, 0, 4, 4]} barSize={20} />
            <Bar dataKey="profit" stackId="a" fill="var(--color-profit)" radius={[0, 0, 0, 0]} barSize={20} />
            <Bar dataKey="earning" stackId="a" fill="var(--color-earning)" radius={[4, 4, 0, 0]} barSize={20} fillOpacity={0.5} />
          </BarChart>
        </ChartContainer>
      </CardContent>
    </Card>
